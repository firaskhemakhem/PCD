...
class FeedBackEntrp extends React.Component {
            ...
            .then((result) => {
                ...
                for (let i = 0; i < result.length; i++) {
                    if (result[i].Recruteur == window.location.href.split('/')[4]) {
                        this.setState({ dataFeedBack: [...this.state.dataFeedBack, result[i]] })
                        this.setState({
                            isLoginFeed: true
                        })
    ...
    render() {
        ...
        const rowsFeed = feedData.map((feed) =>
        (this.state.isLoginFeed && <tr >
            <td width={'273px'}>{feed.FeedBack}</td>
            <td> <StarRatings
                rating={feed.Rating}
